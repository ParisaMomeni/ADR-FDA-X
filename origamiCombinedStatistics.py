...
def origami_star_plot(
    ...
    # Shared/global outer ring override
    if outer_r_override is not None:
        max_ring = float(outer_r_override)
    else:
        max_val  = float(np.nanmax(df.iloc[2:].to_numpy()))
        max_ring = float(np.ceil(max_val * 10) / 10.0)
        if max_ring < 0.1: max_ring = 0.1
    ...
    labels = [label.title() for label in (df.columns if vlabels is None else vlabels)]
...
# ------------------------------------------------------
def load_fda_raw(path, col):
    dfx = pd.read_excel(path)
    dfx = dfx.rename(columns={dfx.columns[0]: "ADR", col: "Prevalence"})
    dfx = dfx[["ADR", "Prevalence"]].dropna()
    dfx["ADR"] = dfx["ADR"].str.lower()
    dfx["Prevalence"] = dfx["Prevalence"].astype(str).str.replace('%','', regex=False).astype(float)
    return dict(zip(dfx["ADR"], dfx["Prevalence"]))

fda_all = Counter()
for v in fda_info.values():
    fda_all.update(load_fda_raw(*v))   # <-- raw prevalence pooled across doses
...
bars1 = ax.bar(x - width/2, twitter_values, width, label='X', color='#00a19a')
bars2 = ax.bar(x + width/2, fda_values, width, label='FDA', color='#ff2d7a')

# Labels and titles
ax.set_ylabel('Prevalence (%)')
# y-label only on the left column panels (top-left and bottom-left)

#ax.set_title('Top-10 Most Common ADRs (X vs FDA)')
ax.set_xticks(x)
ax.set_xticklabels([adr.title() for adr in top_adrs], rotation=45, ha='right', fontsize=18, fontweight='bold')
...
##------------------------------------------------------
# ======================================================
# 4-panel BAR CHARTS (match origami panels)
# ======================================================

def get_topN_vectors_from_dict(dist_dict, axes, eps=EPSILON, renorm=True):
    """Extract values on 'axes' from a distribution dict; optionally renormalize on axes."""
    v = np.array([dist_dict.get(a, 0.0) for a in axes], dtype=float)
    v = np.maximum(v, eps)
    if renorm:
        s = v.sum()
        if s > 0:
...
def get_brand_bar_vectors(brand, axes, twitter_adr_counts, brand_level_fda, eps=EPSILON):
    """Brand-level vectors that match origami_star normalization."""
    t_dict = twitter_adr_counts.get(brand, {})
    f_dict = brand_level_fda.get(brand, {})
    t = get_topN_vectors_from_dict(t_dict, axes, eps=eps, renorm=True)
    f = get_topN_vectors_from_dict(f_dict, axes, eps=eps, renorm=True)
    return t, f

def get_coarse_bar_vectors(axes, twitter_all, fda_all, eps=EPSILON):
    """Coarse-level vectors that match your df_coarse normalization."""
    t = get_topN_vectors_from_dict(twitter_all, axes, eps=eps, renorm=True)
    f = get_topN_vectors_from_dict(fda_all, axes, eps=eps, renorm=True)
    return t, f


# ---- Panel definitions (same as origami) ----
panel_map = {
    "Ozempic": 0,
    "Wegovy": 1,
    "Rybelsus": 2,
...
subplot_titles = {
    "Ozempic": "(a) Ozempic: FDA vs. X",
    "Wegovy": "(b) Wegovy: FDA vs. X",
    "Rybelsus": "(c) Rybelsus: FDA vs. X",
    "Coarse": "(d) Coarse-Level: FDA vs. X"
}

# ---- Build the 2×2 bar-chart figure ----
fig_bar, axs_bar = plt.subplots(2, 2, figsize=(20, 16))
axs_bar = axs_bar.ravel()

xpos = np.arange(len(top_adrs))
width = 0.45

# Optional: set a shared y-limit for visual comparability
# (compute max across all panels)
max_y = 0.0
for brand in ["Ozempic", "Wegovy", "Rybelsus"]:
    t, f = get_brand_bar_vectors(brand, top_adrs, twitter_adr_counts, brand_level_fda)
    max_y = max(max_y, float(t.max()), float(f.max()))
tC, fC = get_coarse_bar_vectors(top_adrs, twitter_all, fda_all)
max_y = max(max_y, float(tC.max()), float(fC.max()))
y_lim = max_y * 1.15  # small headroom
...
# ---- Brand panels ----
for brand in ["Ozempic", "Wegovy", "Rybelsus"]:
    ax = axs_bar[panel_map[brand]]
    t, f = get_brand_bar_vectors(brand, top_adrs, twitter_adr_counts, brand_level_fda)

    b1 = ax.bar(xpos - width/2, t, width, label="X", color="#00a19a")
    b2 = ax.bar(xpos + width/2, f, width, label="FDA", color="#ff2d7a")

    ax.set_title(subplot_titles[brand], fontweight="bold", pad=10)
    ax.set_xticks(xpos)
    ax.set_xticklabels([a.title() for a in top_adrs], rotation=45, ha="right",
    ...
    if shared_handles is None:
        shared_handles = [b1[0], b2[0]]
        shared_labels = ["X", "FDA"]

# ---- Coarse panel ----
ax = axs_bar[panel_map["Coarse"]]
t, f = get_coarse_bar_vectors(top_adrs, twitter_all, fda_all)
# Y-axis label only for left-column panels
axs_bar[0].set_ylabel("Prevalence (%)", fontsize=18, fontweight="bold")
axs_bar[2].set_ylabel("Prevalence (%)", fontsize=18, fontweight="bold")

b1 = ax.bar(xpos - width/2, t, width, label="X", color="#00a19a")
b2 = ax.bar(xpos + width/2, f, width, label="FDA", color="#ff2d7a")

ax.set_title(subplot_titles["Coarse"], fontweight="bold", pad=10)
ax.set_xticks(xpos)
ax.set_xticklabels([a.title() for a in top_adrs], rotation=45, ha="right",
                   fontsize=18, fontweight="bold")
ax.tick_params(axis="y", labelsize=18)
...
# ---- Shared legend (like origami) ----
fig_bar.legend(shared_handles, shared_labels,
...
plt.tight_layout()
plt.savefig("Results/combined_bars_fourpanel.png", dpi=300, bbox_inches="tight")
plt.show()
...
pcols = [["#00a19a", "#ff2d7a"]]*4
pfcols = [["#00a19a11", "#ff2d7a11"]]*4  # very faint fill
pltys  = [[1, 2]]*4
...
# -------------------------------------------------
# GLOBAL max height for consistent outer ring
# -------------------------------------------------
all_vals = []
for brand in brands:
    df_brand = brand_df_for_origami_star(brand, top_adrs, twitter_adr_counts, brand_level_fda)
    all_vals.extend(df_brand.loc["X"].values)
    all_vals.extend(df_brand.loc["FDA"].values)

# Add coarse-level values too
all_vals.extend(df_coarse.loc["X"].values)
all_vals.extend(df_coarse.loc["FDA"].values)

# Compute shared outer ring (e.g. max=0.5 → ring at 0.5)
global_max_val = max(all_vals)
global_outer_ring = float(np.ceil(global_max_val * 10) / 10.0)
print(f"✅ Global outer ring = {global_outer_ring:.2f}")
...
for brand in brands:
    ...
    ax, handles, labels = origami_star_plot(
        ...
        title=f"{subplot_titles[brand]}\n(GOS = {iou:.3f})",
         # ASD = {asd_t_f:.3f})",
        centerzero=False,
        vlabels=top_adrs,
        pcol=pcols[i], plty=pltys[i], plwd=[2, 2],
        pfcol=pfcols[i], ax=axs[i],
        outer_r_override=global_outer_ring,
        return_handles=True, draw_spokes_from_center=True
    )
...
fig.patch.set_facecolor("white")
plt.tight_layout()
plt.savefig("Results/combined_origami_fourpanel.png", dpi=300, bbox_inches="tight")
plt.show()
